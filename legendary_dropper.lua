...
--         local rnd = math.floor(math.random(#legendary_skin_list) + 0.5)
...
--         fake_lootdrop_string = fake_lootdrop_string ..
--             " " .. tostring(fake_global_index) .. "-" .. tostring(fake_item_category) .. "-" .. tostring(fake_item_id)
--       end
...
function IngameLobbyMenuState:set_lootdrop(...)
  if Spoofer and Spoofer._settings.lootdrop_spoof then
    self:new_set_lootdrop(...)
  else
    old_set_lootdrop(self, ...)
  end
end

function IngameLobbyMenuState:new_set_lootdrop(drop_category, drop_item_id, ...)
  ...
  if not Global.game_settings.single_player and managers.network:session() then
    ...
    local drop_list = {}
    for k, v in pairs(tweak_data.economy.safes or {}) do
      table.insert(drop_list, { tostring(k), 'safes' })
    end

    for k, v in pairs(tweak_data.economy.drills or {}) do
      if v['name_id'] then
        table.insert(drop_list, { tostring(k), 'drills' })
      end
    end

    local rnd = math.floor(math.random(#drop_list) + 0.5)
    local fake_item = drop_list[rnd]

    local fake_item_id = fake_item[1]
    local fake_item_category = fake_item[2]
    ...
    managers.network:session():send_to_peers("feed_lootdrop", fake_global_index, fake_item_category, fake_item_id, max_pc,
      item_pc, card_left_pc, card_right_pc)
  end
end
